...
\\\\
We attempted to visualize the clustering/community structure of the graph, using NetworkX's visualization and Community's built-in partition functions. Nodes were sorted and colored based on community. The graph was then displayed using Networkx's `spring layout', which uses Fruchterman-Reingold force-directed algorithm. Nodes modeled as having repulsive forces and edges are modeled as spring. A balance of these forces allows the graph to be laid out with as few overlapping edges as possible. Figure \ref{fig:visualization} shows a coloring of graph clusters. Clusters are distinguishable, though not clearly separatable. 
\\\\
...
\noindent We were able to win on graph 2.10.32, but did very poorly on graph 2.10.31. We visualized and investigated the structure of the graphs to determined why this discrepency occurred.
\\\\
